...
def temp_vector_store():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Mock the paths
        with patch(
            "app.rag.INDEX_PATH", os.path.join(vector_store_path, "index.faiss")
        ), patch(
            "app.rag.META_PATH", os.path.join(vector_store_path, "metadata.npy")
        ), patch(
            "app.cache.CACHE_PATH", os.path.join(vector_store_path, "cache.json")
        ):
...
def mock_openai_response():
    ...
    mock_response.choices[0].message.content = (
        "Based on the policy, coverage is $500,000 per occurrence."
    )
...
def sample_embeddings():
    ...
    return np.random.rand(3, 384).astype(
        "float32"
