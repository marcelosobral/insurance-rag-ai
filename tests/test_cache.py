...
from app.cache import (
    load_cache, save_cache, cosine_similarity, 
    check_cache, add_to_cache
)
...
class TestCache:
    ...
    def test_load_cache_file_exists(self):
        ...
        with patch('os.path.exists', return_value=True), \
             patch('builtins.open', mock_open(read_data=json.dumps(test_cache))):
            cache = load_cache()
    ...
    def test_check_cache_hit(self, temp_vector_store):
        ...
        with patch('app.cache.load_cache', return_value=test_cache), \
             patch('app.cache.embed_query', return_value=query_embedding):
    ...
    def test_add_to_cache(self, temp_vector_store):
        ...
        with patch('app.cache.load_cache', return_value=existing_cache), \
             patch('app.cache.embed_query', return_value=np.array(embedding)), \
