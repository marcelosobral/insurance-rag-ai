...
from app.ingest import load_documents, chunk_text, build_index
...
class TestIngest:
    ...
    def test_build_index_success(self, temp_vector_store):
        ...
        with patch('app.ingest.load_documents', return_value=(sample_docs, sample_metadata)), \
             patch('app.ingest.embed_texts', return_value=sample_embeddings), \
    ...
    def test_build_index_empty_documents(self, temp_vector_store):
        ...
        with patch('app.ingest.load_documents', return_value=([], [])), \
             patch('app.ingest.embed_texts', return_value=np.array([]).reshape(0, 384)), \
             patch('faiss.write_index') as mock_write_index, \
             patch('numpy.save') as mock_save:
    ...
    def test_full_ingestion_pipeline(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with open(test_file, 'w') as f:
                f.write("This policy provides comprehensive coverage for bodily injury and property damage.")
            
            index_path = os.path.join(vector_store_dir, "index.faiss")
            meta_path = os.path.join(vector_store_dir, "metadata.npy")
            
            with patch('app.ingest.DATA_DIR', data_dir), \
                 patch('app.ingest.INDEX_PATH', index_path), \
    ...
    def test_build_index_file_permissions(self, temp_vector_store):
        ...
        with patch('app.ingest.load_documents', return_value=(sample_docs, sample_metadata)), \
             patch('app.ingest.embed_texts', return_value=sample_embeddings), \
             patch('faiss.write_index', side_effect=PermissionError("Access denied")):
