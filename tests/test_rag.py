...
from app.rag import load_index, retrieve, generate_answer
...
class TestRAG:
    ...
    @pytest.mark.unit  
    def test_retrieve_success(self, temp_vector_store, mock_index_and_metadata, sample_embeddings):
        """Test successful retrieval"""
        mock_index, mock_metadata = mock_index_and_metadata
        query_embedding = sample_embeddings[0]  # Use first embedding as query
        
        with patch('app.rag.load_index', return_value=(mock_index, mock_metadata)), \
    ...
    def test_retrieve_no_results(self, temp_vector_store):
        ...
        with patch('app.rag.load_index', return_value=(empty_index, empty_metadata)), \
             patch('app.rag.embed_query', return_value=np.random.rand(dimension).astype('float32')):
    ...
    @pytest.mark.unit
    def test_generate_answer_cache_miss(self, temp_vector_store, mock_openai_response, sample_documents):
        ...
        with patch('app.rag.check_cache', return_value=(None, False)), \
             patch('app.rag.retrieve', return_value=retrieval_result), \
             patch('app.rag.client.chat.completions.create', return_value=mock_openai_response), \
             patch('app.rag.add_to_cache') as mock_add_cache:
    ...
    def test_confidence_calculation(self, temp_vector_store):
        ...
        for avg_score, expected_min_conf in test_cases:
            ...
            with patch('app.rag.check_cache', return_value=(None, False)), \
                 patch('app.rag.retrieve', return_value=retrieval_result), \
                 patch('app.rag.client.chat.completions.create', return_value=mock_response), \
                 patch('app.rag.add_to_cache'):
    ...
    def test_full_rag_pipeline(self, temp_vector_store, sample_embeddings, sample_documents):
        ...
        mock_response = Mock()
        mock_response.choices = [Mock()]
        mock_response.choices[0].message.content = "Based on the policy, coverage includes bodily injury protection."
        
        with patch('app.rag.load_index', return_value=(index, metadata)), \
             patch('app.rag.embed_query', return_value=sample_embeddings[0]), \
             patch('app.rag.check_cache', return_value=(None, False)), \
             patch('app.rag.client.chat.completions.create', return_value=mock_response), \
             patch('app.rag.add_to_cache'):
